...
def count_letter_occurrences(letter, word: str):
    split_word = list(word)
    num=0
    for i in range(len(split_word)):
        if letter in split_word[i-1]:
            num=num+1
    return num
...
def alphabet_display_with_guessed_letters_marked(letters_alphabet, guessed_letters):
    for letter in letters_alphabet:
        if letter in guessed_letters:
           ...
           letters_alphabet.add(new_iten)

    return letters_alphabet
...
def update_letters_to_be_guessed(hidden_letters, letter):
    for i in range (len(hidden_letters)):
        if hidden_letters[i-1] == letter:
            hidden_letters[i-1].remove(letter)
